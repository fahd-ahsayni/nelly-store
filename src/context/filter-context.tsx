...
export function FilterProvider({
  children,
  allProducts: productsFromProps, // Renamed prop for clarity in this scope
}: {
  children: ReactNode;
  allProducts?: Product[]; // This is the prop name from the component's signature
}) {
  ...
  // Effect to synchronize productsFromProps to the internal allProducts state
  useEffect(() => {
    setAllProducts(productsFromProps || []);
  }, [productsFromProps]);
          ...
          if (filterState.isInStock !== null) {
            ...
            const productInStock =
              typeof product.inStock === "string"
                ? (product.inStock as unknown as string).toLowerCase() ===
                  "true"
                : !!product.inStock;
  ...
  // Update all products from outside (e.g. from SupabaseContext via props or this function)
