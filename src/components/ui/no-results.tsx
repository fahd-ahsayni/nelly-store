...
export default function NoResults({ onRetry, isLoading = false }: NoResultsProps) {
  ...
  const appliedFilters = [
    ...
    (filterState.priceRange.min > 0 || filterState.priceRange.max < 1000) ? 'price range' : null,
    ...
    <div className="flex flex-col items-center justify-center py-16 px-4 text-center">
      {isLoading ? (
        <>
          ...
          <h3 className="text-lg font-medium text-zinc-900">Loading products...</h3>
        </>
      ) : (
        <>
          ...
          {appliedFilters.length > 0 ? (
            <>
              <p className="text-zinc-500 mb-6">
                ...
                <span className="font-medium"> {appliedFilters.join(', ')}</span>
              </p>
              
              <p className="text-zinc-700 mb-2">
                Try adjusting your filters or removing some to see more products.
              </p>
              
              {totalProductCount > 0 && (
                <p className="text-sm text-zinc-500">
                  There {totalProductCount === 1 ? 'is' : 'are'} {totalProductCount} product{totalProductCount !== 1 ? 's' : ''} in the database that {totalProductCount !== 1 ? 'are' : 'is'} being filtered out.
                </p>
              )}
            </>
          ) : (
